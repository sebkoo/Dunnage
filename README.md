...
| **4b. Transport and data plane** | The same transport, against the real plane and S3: the bucket exists, ADR-0006 §4's six assumptions and ADR-0007's three are checked by a recorded contract run, and O-10's recovery is decided. | in progress — 14 named tests: O-10's recovery, decided in Core and the driver, its negative control, and the four values a deploy reports. A tree that names no account, the gated contract run and the recorded run are not built |
